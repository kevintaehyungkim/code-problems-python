...
class Game: 
    ...
    def purchase_card(self, p_name, card): 
        ...
        player = Player(self.players.get(p_name))
        # print p
        # print p.to_string()
        ...
        if can_purchase: 
            new_p_resources = self.make_transaction(p_resources, card.price)
            player.resources = new_p_resources
...
new_game = Game(["p1"])  
p1 = new_game.players["p1"].set_resources({'Blue': 4, 'Red': 3})
